...
public interface VendorCouponsRepository  extends PagingAndSortingRepository<VendorCoupons,Long> {
	...
	@Query("select "
			...
			+ " from VendorCoupons c where ((c.fromDate <= (:current_time) and c.toDate >=(:current_time)) or (:userId)!=0L) and (c.id not in (:couponIds)) and (c.couponType.id = (:couponType) or ((:couponType) = 0 and c.couponType.id!=1 and c.couponType.id!=2)) and (c.vendor.user.userId = (:userId) or (:userId) = 0) order by id desc")
	List<VendorCouponsDTO1> findByLatest1(@Param("userId") Long userId, @Param("couponType") Long couponType, @Param("customerId") Long customerId , @Param("couponIds") List<Long> couponIds,@Param("current_time") Timestamp current_time,Pageable pageable);
}
